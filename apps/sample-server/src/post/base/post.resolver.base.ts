...
export class PostResolverBase {
  ...
  async createPost(@graphql.Args() args: CreatePostArgs): Promise<Post> {
    return await this.service.createPost({
      ...
      data: {
        ...
        authors: args.data.authors
          ? {
              connect: args.data.authors,
            }
          : undefined,
  ...
  async getAuthors(@graphql.Parent() parent: Post): Promise<Author | null> {
    const result = await this.service.getAuthors(parent.id);
  ...
  async getComments(@graphql.Parent() parent: Post): Promise<Comment | null> {
